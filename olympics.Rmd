...
Plot of top 10 total medals per countries per capita compared to top 10 total medals countries.
```{r}
population %>% 
  filter(year == 2013) %>% 
  left_join(medals %>% 
  ...
  drop_na() %>% 
  mutate(medals_per_capita = total / population * 100000) %>% 
  top_n(10, wt = medals_per_capita)
  ggplot(aes(medals_per_capita, fill = country)) +
  geom_bar() 
```
